...
export default function DashboardHome() {
  ...
  useEffect(() => {
    const fetchCalves = async () => {
      try {
        const response = await axios.get("/api/calves");
  ...
  return (
    <div className="min-h-screen container mx-auto px-8 py-6 bg-gray-950 text-white">
      {/* Header */}
...
>
  ...
  <div className="p-2 bg-gray-900/60 backdrop-blur-sm absolute bottom-0 w-full">
    <h3 className="text-md font-semibold text-white truncate">{cow.name}</h3>
...
>
  ...
  />
  <div className="p-2 bg-gray-900/60 backdrop-blur-sm absolute bottom-0 w-full">
    <h3 className="text-md font-semibold text-white truncate">{calf.name}</h3>
